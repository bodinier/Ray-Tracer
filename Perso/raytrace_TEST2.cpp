...
bool hitPlan(const ray &r, const plan &p, float &t)
{
    vecteur n = p.normal;
    n = n/sqrtf(n*n);
    float A = (p.origin - r.start)*(r.dir);
    float B = n * r.dir;
    float t_calc = -B/A;

    if (t_calc < t && t_calc > 0.0f)
    {
        t = t_calc;
        return true;
    }
...
{ 
    ...
    bool retvalue = false;  // Si retvalue == true , il faut actualiser un pixel sinon, il n'y a rien a faire
    ...
    { 
        for (int x = 0; x < myScene.sizex; x++ ) 
        {
            ...
            { 
                ...
                float t = 20000.0f; // on part du fond de la scene 
                int currentSphere= -1;
                int currentPlan = -1;
                int obj_type;
                ...
                if (currentSphere == -1)
                {
                 ...
                 // impact est le point d'intersection du rayon et de l'objet

                if(obj_type == 1) // ====================== Intersection avec sphere ======================
                    {
                        ...
                        for (unsigned int j = 0; j < myScene.lgtTab.size(); ++j) 
                        {
                            ...
                            lightRay.dir = (1/t) * dist; // On pondere selon la distance a la lampe
                    ...
                    if (obj_type == 2) // ====================== Intersection avec plan ======================
                    { 
                        point impact = viewRay.start + t * viewRay.dir;
                        vecteur n = myScene.planTab[currentPlan].normal ; // normale au point d'intersection
                        n = n / sqrtf(n*n); // On normalise n
...
int main(int argc, char* argv[]) 
{
    ...
    std::cout << "nb plans = " << myScene.planTab.size() << std::endl;
