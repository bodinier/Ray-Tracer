...
point find_intersection_sphere_i(scene &myScene, ray &viewRay, float &t, int currentSphere)
{
    bool test = hitSphere(viewRay, myScene.sphTab[currentSphere], t);
    point impact = viewRay.start + t * viewRay.dir;
     
    return impact;
}

ray refract_ray_plan(scene &myScene, ray &viewRay, float &t, int currentPlan, point &impact)
{
    // renvoie le rayon refracte qui traverse un plan.
    vecteur n = myScene.planTab[currentPlan].normale; // normale au point d'intersection
    n = n / sqrtf(n*n);
    material currentMat = myScene.matTab[myScene.planTab[currentPlan].material]; 
    float ind2 = currentMat.refraction; // indice du materiau
    float cos_theta_i = viewRay.dir * n;
    float sin_theta_t = ind1/ind2 * (1 - cos_theta_i*cos_theta_i);

    // On genere le rayon transmis
    ray t_ray;
    t_ray.start = impact;
    t_ray.dir = ind1/ind2 * viewRay.dir - (ind1/ind2*cos_theta_i + sqrtf(1 - sin_theta_t*sin_theta_t )) * n ;
    return t_ray;
}
...
ray refract_ray_sphere(scene &myScene, ray &viewRay, float &t, int currentSphere, point &impact)
{
    ...
    // On cherche la deuxieme intersection avec l'autre face de la sphere :
    float newT = t+1;
    point impact2 = find_intersection_sphere_i(myScene, t_ray, newT, currentSphere);
    vecteur n2 = impact2 - myScene.sphTab[currentSphere].pos; // normale au point d'impact
    n2 = n2 / sqrtf(n2*n2);

    float cos_theta_i2 = t_ray.dir * n2;
    float sin_theta_t2 = ind2/ind1 * (1 - cos_theta_i2*cos_theta_i2);

    // On genere le 1er rayon transmis
    ray t_ray2;
    t_ray2.start = impact2;
    t_ray2.dir = ind2/ind1 * t_ray.dir - (ind2/ind1*cos_theta_i2 + sqrtf(1 - sin_theta_t2*sin_theta_t2 )) * n2 ;
    return t_ray2;
            ...
            do { 
                ...
                switch (obj_type)
                {
                    case 1 :
                        ...
                        if (myScene.matTab[myScene.sphTab[currentSphere].material].opacity != 1)
                        {
                            ...
                            if (level == 0)
                                ...
                                refract = refract_ray_sphere_tmp(myScene, viewRay, t, currentSphere, impact);
                    ...
                    case 2 :
                        ...
                        if (myScene.matTab[myScene.planTab[currentPlan].material].opacity != 1)
                        {
                            /*ray refract = refract_ray_plan(myScene, viewRay, t, currentPlan, impact);
                            pix_impactPlan(myScene, red, green, blue, coef, refract, t, currentPlan, impact);*/
                        }
                        level++;
                        break;
